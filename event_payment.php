...
$sel="select * from ebooking where booking_id=$id";
$ex=$con->query($sel);
$reco=$ex->fetch_object();
//print_r($reco);
if (isset($_REQUEST['event_payment'])) 
{
	...
	{
		$ins="insert into payment (e_id,cus_id,payment_date,booking_id,total_amount,payment_method,card_number,cvv,status) values ('$event_id','$user_id','$date','$book_id','$total','$method','$card_num','$crd_cvv','paid')";

	}
	else
	{
		$ins="insert into payment (e_id,cus_id,payment_date,booking_id,total_amount,payment_method,status) values ('$event_id','$user_id','$date','$book_id','$total','$method','pending')";
...
<center>
	...
	<form method="post" style=" position: absolute;  left: 250px; width: 500px; background-color: grey"><br>
		<label>Select ur payment methods</label>
		<br>
		...
		<br>
		<br>
		<input type="submit" name="event_payment" value="Payment" class="btn btn-success">
	</form>
</div>
</center>